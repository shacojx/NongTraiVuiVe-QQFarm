<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('admin/tpl/album|admin/tpl/header|admin/tpl/side|admin/tpl/footer|template/default/header|template/default/footer', '1315016507', 'admin/tpl/album');?>﻿<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
...
<option value="0"<?php if($_GET['friend'] == '0') { ?> selected<?php } ?>>Users can see the whole station</option>
<option value="1"<?php if($_GET['friend'] == '1') { ?> selected<?php } ?>>All friends can see that</option>
<option value="2"<?php if($_GET['friend'] == '2') { ?> selected<?php } ?>>Only designated friends can be seen</option>
<option value="3"<?php if($_GET['friend'] == '3') { ?> selected<?php } ?>>Itself can be seen only</option>
<option value="4"<?php if($_GET['friend'] == '4') { ?> selected<?php } ?>>View with a password</option>
</select>	
</td>
</tr>
<tr>
...
<input type="<?php if($allowbatch) { ?>checkbox<?php } else { ?>radio<?php } ?>" name="ids[]" value="<?=$value['albumid']?>"> Choose
</td>
<td>
...
<?php } ?>
<br />Time: <?php echo sgmdate('Y-m-d',$value[dateline]); ?>
<?php if($value['friend']) { ?><br />Permissions: [<a href="admincp.php?ac=album&friend=<?=$value['friend']?>"><?=$value['friend']?></a>]<?php } ?>
