<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('admin/tpl/comment|admin/tpl/header|admin/tpl/side|admin/tpl/footer|template/default/header|template/default/footer', '1315016504', 'admin/tpl/comment');?>﻿<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
...
<option value="uid"<?=$idtype['uid']?>>Message</option>
<option value="blogid"<?=$idtype['blogid']?>>Log</option>
<option value="picid"<?=$idtype['picid']?>>Picture</option>
<option value="eventid"<?=$idtype['eventid']?>>Activities</option>
<option value="sid"<?=$idtype['sid']?>>Share</option>
</select>
<th>Comment ID</th><td><input type="text" name="id" value="<?=$_GET['id']?>"></td>
</tr>
<tr><th>Comments UID</th><td><input type="text" name="authorid" value="<?=$_GET['authorid']?>"></td>
<th>Reviewer</th><td><input type="text" name="author" value="<?=$_GET['author']?>"></td>
</tr>
<?php if($allowmanage) { ?>
<tr><th>Post, UID</th><td colspan="3"><input type="text" name="uid" value="<?=$_GET['uid']?>"></td></tr>
<?php } ?>
...
<th>Release IP</th><td><input type="text" name="ip" value="<?=$_GET['ip']?>"></td>
</tr>
...
<option value="dateline"<?=$orderby['dateline']?>>Comment Time</option>
</select>
<select name="ordersc">
...
<p>
<?php if($value['author']) { ?>
Author:<a href="admincp.php?ac=comment&author=<?=$value['author']?>"><?=$_SN[$value['authorid']]?></a>
<?php } else { ?>
Author:<a href="space.php?uid=<?=$value['authorid']?>" target="_blank">Anonymous</a>
<?php } ?>
Post, object: <a href="admincp.php?ac=comment&id=<?=$value['id']?>&idtype=<?=$value['idtype']?>"><?=$value['idtype']?>-<?=$value['id']?></a>
<?php if($value['idtype'] == 'uid') { ?>
(<a href="space.php?uid=<?=$value['id']?>&do=wall" target="_blank">Access</a>)
<?php } ?>
