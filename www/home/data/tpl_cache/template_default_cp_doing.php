<?php if(!defined('IN_UCHOME')) exit('Access Denied');?><?php subtplcheck('template/default/cp_doing|template/default/header|template/default/space_doing_li|template/default/space_doing_form|template/default/footer', '1315016769', 'template/default/cp_doing');?><?php if(empty($_SGLOBAL['inajax'])) { ?>
...
<p>Bạn muốn xóa các bản ghi ?</p>
<p class="btn_line">
<input type="hidden" name="refer" value="<?=$_SGLOBAL['refer']?>">
<button name="deletesubmit" type="submit" class="submit" value="true">OK</button>
</p>
<input type="hidden" name="formhash" value="<?php echo formhash(); ?>" />
</form>
</div>
...
<a href="#" id="do_face_<?=$doid?>_<?=$id?>" title="Chèn Smiley" onclick="showFace(this.id, 'do_message_<?=$doid?>_<?=$id?>');return false;"><img src="image/facelist.gif" align="absmiddle" /></a>
...
<input type="button" name="do_button" class="submit" id="docommform_btn_<?=$doid?>_<?=$id?>" onclick="ajaxpost('docommform_<?=$doid?>_<?=$id?>', 'docomment_get', 1)" value="Trả lời">
<button type="button" name="btncancel" class="button" onclick="docomment_form_close(<?=$doid?>, <?=$id?>);">Hủy</button>
<input type="hidden" name="formhash" value="<?php echo formhash(); ?>" />
</form>
...
<a href="javascript:;" onclick="docomment_form(<?=$value['doid']?>, <?=$value['id']?>);" class="re">Trả lời</a>
<?php if($value['uid']==$_SGLOBAL['supe_uid'] || $dv['uid']==$_SGLOBAL['supe_uid']) { ?> <a href="cp.php?ac=doing&op=delete&doid=<?=$value['doid']?>&id=<?=$value['id']?>" id="doing_delete_<?=$value['doid']?>_<?=$value['id']?>" onclick="ajaxmenu(event, this.id)" class="gray">Xóa</a><?php } ?>
...
<div class="r_option">Còn lại <strong id="maxlimit">200</strong> ký tự</div>
...
Trả lời câu hỏi<?php question(); ?> 
<?php } else { ?>
Nhập mã xác nhận<script>seccode();</script> 
<?php } ?>
...
<button type="submit" id="add" name="add" class="post_button">Post</button>
