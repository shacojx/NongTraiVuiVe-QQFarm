...
#Necessary experience and money to upgrade
...
if($redland >=18) {
	die('{"code":888,"direction":"Chúc mừng bạn đã nâng cấp tất cả lên đất đỏ thành công!"}');
}

#Access to upgrade the position of the red land
...
if($_REQUEST['confirm'] == 1) {
	...
	if($reclaim < 18) {
		$ta='Bạn cần mở rộng hết các ô đất trước đã';
	}
	if($money < $upmoney && $level < $uplevel) {
		$ta='Bạn chưa đủ điều kiện yêu cầu!';
	} else {
		$level < $uplevel && $ta='Chưa đủ cấp độ!';
		$money < $upmoney && $ta='Không đủ tiền!';
	} 
