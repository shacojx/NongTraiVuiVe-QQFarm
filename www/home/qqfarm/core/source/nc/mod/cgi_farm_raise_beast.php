...
foreach($beastsinfo['info'] as $key => $value) {
	if($value['fid']==$_QFG['uid'] && $beasts['beasts'][$slotid]['status'] !=2 ){
		if((int)$value['isfarm'] == 1) die('{"direction":"Thả thú hoang (người khác sẽ nhìn thấy)","ecode":-30306}');
	}
}

$beastsinfo['info'][] = array("attack"=>array(),"fid"=>$_QFG['uid'],"id"=>$slotid,"isfarm"=>1);
